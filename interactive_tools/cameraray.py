...
def getCameraMouseRay(camera, mouse_pos):
    ...
    v_cam_forward = math_utils.p3d_to_np(render.getRelativeVector(camera, camera.node().getLens().getViewVector()))
    v_cam_forward = v_cam_forward / np.linalg.norm(v_cam_forward)
    # self.camera_gear.camera.node().getLens().getViewVector()

    v_cam_up = math_utils.p3d_to_np(render.getRelativeVector(camera, camera.node().getLens().getUpVector()))
