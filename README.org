...
- [ ] *load and play an external file into an EdgePlayer*
  - [ ] make EdgePlayerSM (state machine) and call the routines of PlaybackerSM and GraphickerSM from inside of there
  - [ ] implement mouse hovering and pickig in the EdgePlayerSM
  ...
  - [ ] TODO: Implement the naive approach of calculating the edge length from the duration; worry about node dragging of nodes in re-merging paths later
