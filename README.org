...
- [ ] make a draw on screen (2d plane) functionality
  ...
  - [ ] implement sketching on this plane
    - [ ] restrict sketching to only the plane boundaries
      - [ ] TODO clip everything over the plane boundary
      - [ ] TODO Panner2d: fix: ~view_distance_0~ doesn't do what it implies and affects zooming range
      - [ ] TODO When DraggableResizableDrawableOnFrame and PDFViewer overlap, I can't draw on DraggableResizableDrawableOnFrame or drag it. There is a double intersection when picking
- [X] display a frame2d with the time-domain data of the audio file
- +[ ] make an interactive frame2d with a vertical line to interactively select an x-value+
