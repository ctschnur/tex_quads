...
****** TODO make an object picker through collision detection
- basic picker implemented: pick.py
  -> now generalize the picking to an orbiter camer
******* TODO onPress: save viewing plane (in 3d), then calculate new position from dragging within that plane
