...
class Recorder_for_live_updating:
    ...
    def grab_last_frames(self, num=1):
        """ """
        if len(self.frames) > 0:
            if len(self.frames) >= num:
    ...
    def record_blocking_mode(self):
        ...
        stream = p.open(format=Recorder_for_live_updating.FORMAT,
                            channels=Recorder_for_live_updating.CHANNELS,
                            rate=Recorder_for_live_updating.RATE,
                            input=True,  # input stream: recording
                            frames_per_buffer=Recorder_for_live_updating.CHUNK,
        ...
        # for i in range(0, int(Recorder_for_live_updating.RATE / Recorder_for_live_updating.CHUNK * Recorder_for_live_updating.RECORD_SECONDS)):
        ...
        while True:
            if (self.is_recording_query_function() == True):
                ...
                data = stream.read(Recorder_for_live_updating.CHUNK)
        ...
        for startp, stopp in zip(start_positions, stop_positions):
            ...
            if ((stopp - Recorder_for_live_updating.num_of_chunks_to_cut_away_end)
                > (startp + Recorder_for_live_updating.num_of_chunks_to_drop_start)):
                frames_new[stopp - Recorder_for_live_updating.num_of_chunks_to_cut_away_end:stopp] = []
                frames_new[startp:startp + Recorder_for_live_updating.num_of_chunks_to_drop_start] = []
